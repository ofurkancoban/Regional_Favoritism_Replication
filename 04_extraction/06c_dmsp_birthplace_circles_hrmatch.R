...
#
# Output: data/processed/ntl/dmsp_circles_panel_hrmatch.csv
...
sf::sf_use_s2(FALSE)
circles <- sf::st_read("data/processed/birthplace_circles_5km_hrmatch.gpkg", quiet = TRUE)
sf::st_geometry(circles) <- "geometry"
cat(sprintf("Circles: %d\n", nrow(circles)))

out_dir <- "data/processed/ntl/dmsp_circles_by_year_hrmatch"
extract_ntl_panel(
  polygons   = circles,
  id_col     = "circle_id",
  extra_cols = "gid_0",
  ...
  value_col  = "dmsp_ntl",
  out_dir    = out_dir,
  panel_name = "data/processed/ntl/dmsp_circles_panel_hrmatch.csv"
)
