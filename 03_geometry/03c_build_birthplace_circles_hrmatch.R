...
#
# Output: data/processed/birthplace_circles_5km_hrmatch.gpkg
#   Columns: circle_id, gid_0, geometry (clipped 5km buffer, WGS84)
#   Also: data/processed/birthplace_circles_lookup_hrmatch.csv
...
cat("=== Step 1: Collect birthplace points (PLAD ONLY, HR exact sample: 126 countries + pop>500k + 1992-2009 overlap) ===\n")
...
plad <- plad[gid_0 %in% hr126$iso3 & !gid_0 %in% hr_small$iso3]
plad <- plad[startyear <= 2009 & endyear >= 1992]
...
data.table::fwrite(pts[, .(leader, iso3, startyear, endyear, circle_id)],
                    "data/processed/birthplace_circles_lookup_hrmatch.csv")
...
# vs. the old method's severe distortion at the same latitude).
sf::sf_use_s2(TRUE)
...
sf::st_write(circles_wgs84, "data/processed/birthplace_circles_5km_hrmatch.gpkg", quiet = TRUE, delete_dsn = TRUE)
cat("Saved: data/processed/birthplace_circles_5km_hrmatch.gpkg\n")
cat("Saved: data/processed/birthplace_circles_lookup_hrmatch.csv\n")
