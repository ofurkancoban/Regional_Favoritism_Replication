...
# the exact list.
#
# Two analysis phases after preprocessing: `core` builds the tables and
# figures the main paper (paper.qmd) uses; `supplementary` builds the
# additional tables supplementary.qmd uses (Table III, V, VI, VII), which
# the main paper does not need. `render` renders both documents together
# (Stage 40 covers paper.qmd and supplementary.qmd in one script), so
...
help:
	...
	@echo "  make core          : Run stages 22-32 (main paper's tables, extension, figures)"
...
core:
	@echo "Running Core Analysis Stages (22-32): main paper tables & figures..."
