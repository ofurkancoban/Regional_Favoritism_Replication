...
#
# HR exact-sample restriction (2026-08-20): drop countries with avg
# population < 500,000 (HR's 126-country list) and ADM2 regions entirely
# above 65N latitude, matching HR's own stated exclusion criteria (p. 1001)
# -- same restriction already applied to the other hr_exact_sample/
# scripts, so Table I's own descriptive stats are computed over exactly
# the same estimation sample the regression tables use.
#
...
data.table::fwrite(region_dt, "data/processed/ntl/table1_descriptive_region_hrsample.csv")
data.table::fwrite(cy_dt, "data/processed/ntl/table1_descriptive_countryyear_hrsample.csv")
