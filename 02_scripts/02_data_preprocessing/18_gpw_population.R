...
#
# Description:   Aggregates CIESIN Gridded Population of the World v4 (GPWv4) to ADM2 region-year population, for Table II Column (7)'s per-capita specification.
#
# Inputs:        GPWv4 rasters (NASA Earthdata)
# Outputs:       01_datasets/processed/population_adm2_gpwv4.csv
# ==============================================================================

source(here::here("02_scripts", "02_data_preprocessing", "00_import.R"))
source(here::here("02_scripts", "04_tools", "utils.R"))

gpw_dir <- here::here("01_datasets/raw/GPWv3_pcount")
gpw_years <- c(90L, 95L)              # GPWv3 filename year codes (2000 comes from GPWv4 instead)
gpw_years_full <- c(1990L, 1995L)

gpw4_dir <- here::here("01_datasets/raw/gpw/population/pop")
...
extract_country_year <- function(gid0, zip_code, yr_code) {
  ...
  # exist when exactextractr::exact_extract() actually reads pixel values -- unlink only
...
data.table::fwrite(panel[, .(GID_2, GID_0, year, pop_count, lnpop)],
                   here::here("01_datasets/processed/population_adm2.csv"))
