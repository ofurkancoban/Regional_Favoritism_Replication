# ==============================================================================
# File:          21_build_analysis_panel.R
...
#
# Description:   Assembles the region-year analysis panel: merges nighttime lights, birth-region flags, population, and regional GDP into the single panel every regression table is estimated from.
#
# Inputs:        01_datasets/processed/ntl/dmsp_adm2_panel.csv and covariate outputs above
# Outputs:       01_datasets/processed/analysis_panel.csv
# ==============================================================================
...
ntl <- data.table::fread(here::here("01_datasets/processed/ntl/dmsp_adm2_panel.csv"))
...
cat(sprintf("NTL ADM2: %d rows | %d regions | %d countries\n",
    nrow(ntl), data.table::uniqueN(ntl$gid_2), data.table::uniqueN(ntl$iso3)))
...
cat(sprintf("Wikidata supplement: %d additional country-year rows | %d countries\n",
    nrow(wd_supp_yr), data.table::uniqueN(wd_supp_yr$gid_0)))
...
cat(sprintf("Countries with leader data: %d\n", ntl[has_leader == 1, data.table::uniqueN(iso3)]))
...
pop <- data.table::fread(here::here("01_datasets/processed/population_adm2.csv"))
