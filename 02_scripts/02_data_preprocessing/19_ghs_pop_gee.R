...
#
# Description:   Extracts GHS-POP population benchmarks (1990-2020, 5-year steps) to ADM2 via Google Earth Engine, for interpolation to an annual series covering the 1992-2023 extension window.
#
# Inputs:        Authenticated Google Earth Engine session (reticulate)
# Outputs:       01_datasets/processed/population_adm2_ghspop_vps.csv
# ==============================================================================
...
adm <- sf::st_read(here::here("01_datasets/raw/gadm_3.6/gadm36_levels.gpkg"), layer = "level2", quiet = TRUE)
...
out_dir <- here::here("01_datasets/processed/ntl/ghs_pop_gee_by_country")
...
reduce_chunk <- function(sub) {
  ...
  data.table::rbindlist(lapply(feats, function(f) {
    p <- f$properties
    data.table::as.data.table(p[c("GID_0", "GID_1", "GID_2", sprintf("y%d", years))])
  }), fill = TRUE)
}
...
reduce_with_retry <- function(sub, depth = 0) {
  ...
  data.table::rbindlist(list(reduce_with_retry(sub[seq_len(mid), ], depth + 1),
...
process_country <- function(iso3) {
  ...
  data.table::fwrite(country_result, out_csv)
}
...
wide <- data.table::rbindlist(lapply(files, fread), fill = TRUE)
...
data.table::fwrite(panel[, .(GID_2, GID_0, year, pop_count, lnpop)],
       here::here("01_datasets/processed/population_adm2_ghspop.csv"))
