...
#
# Description:   Table IV Column (1): 5km-radius birthplace-circle regression, on the authors' exact sample.
#
# Inputs:        01_datasets/processed/ntl/dmsp_circles_panel.csv, reference_data/
# Outputs:       01_datasets/processed/table4_col1_own_hrsample.csv
# ==============================================================================
...
lookup <- data.table::fread(here::here("01_datasets/processed/birthplace_circles_lookup.csv"))
ntl <- data.table::fread(here::here("01_datasets/processed/ntl/dmsp_circles_panel.csv"))
...
sf::sf_use_s2(FALSE)
circ_geom <- sf::st_read(here::here("01_datasets/processed/birthplace_circles_5km.gpkg"), quiet = TRUE)
...
cat("\n=== Export Col(1) own-estimate row (feeds table4_assemble_hrsample.R) ===\n")
# Within R2, HR/Stata xtreg-fe convention: 1 - deviance(m) / deviance(a null
# model with only the circle fixed effect). Same convention as Table II's
# own wr2() (table2_own_hrsample.R) and the extension script -- NOT
# fixest::r2(m, "wr2"), which nets out both fixed effects and is
# mechanically near zero for a single binary regressor.
wr2 <- function(m, dd, yvar, unit) {
  null_dev <- stats::deviance(fixest::feols(
    stats::as.formula(sprintf("%s ~ 1 | %s", yvar, unit)), data = dd
  ))
  1 - stats::deviance(m) / null_dev
}
pv1 <- summary(m1)$coeftable[1, 4]
sig1 <- ifelse(pv1 < 0.01, "***", ifelse(pv1 < 0.05, "**", ifelse(pv1 < 0.10, "*", "")))
own_row_col1 <- data.table::data.table(
  col = 1, b = cf1, se = se1, sig = sig1,
  n_regions = data.table::uniqueN(d_fe$circle_id),
  n_obs = nobs(m1),
  within_r2 = wr2(m1, d_fe, "ln_ntl", "circle_id"),
  source = "own"
)
data.table::fwrite(own_row_col1, file.path(out_dir, "table4_col1_own_hrsample.csv"))
cat(sprintf("Saved: %s\n", file.path(out_dir, "table4_col1_own_hrsample.csv")))
