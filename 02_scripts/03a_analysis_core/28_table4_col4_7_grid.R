...
#
# Description:   Table IV Columns (4)-(7): equal-area grid-cell regressions at 50/100/200/400km, on the authors' exact sample.
#
# Inputs:        01_datasets/processed/ntl/dmsp_grid<res>km_panel.csv, reference_data/
# Outputs:       01_datasets/processed/table4_col4_7_own_hrsample.csv
# ==============================================================================
...
cat(sprintf("PLAD country-years with a birthplace-located leader: %d (%d countries)\n",
    nrow(plad_yr), data.table::uniqueN(plad_yr$gid_0)))
...
wd_coords <- data.table::fread(here::here("01_datasets/processed/wikidata_supplement_coords.csv"))
...
cat(sprintf("Wikidata supplement: %d additional country-years | %d countries\n",
    nrow(wd_coords_yr), data.table::uniqueN(wd_coords_yr$gid_0)))
...
run_grid_regression <- function(res, restrict_iso3 = NULL, exclude_above65n = FALSE) {
  gpkg_path <- here::here(sprintf("01_datasets/processed/grid_cells_%dkm.gpkg", res))
  panel_path <- here::here(sprintf("01_datasets/processed/ntl/dmsp_grid%dkm_panel.csv", res))
  if ((!file.exists(gpkg_path) && !file.exists(paste0(gpkg_path, ".gz"))) || !file.exists(panel_path)) {
    cat(sprintf("  MISSING input file(s) for %d km -- skipping\n", res))
    return(NULL)
  }
  ensure_decompressed(gpkg_path)
  ...
  cat(sprintf("  Grid panel (leader-country-years only): %d rows | %d cells | %d countries\n",
      nrow(gpanel), data.table::uniqueN(gpanel$grid_id), data.table::uniqueN(gpanel$gid_0)))
  cat(sprintf("  is_birthregion_grid = TRUE: %d cell-years\n", gpanel[is_birthregion_grid == TRUE, .N]))

  if (data.table::uniqueN(gpanel$gid_0) < 2 || nrow(gpanel) == 0) {
  ...
  # n_regions convention matches Table II / Table IV Col(1)-(3): unique
  # units in the panel object passed to feols, not fixest's own post-fit
  # fixed-effect level count. d_fe itself is attached too (grid_id + ln_ntl
  # only, to keep the saved model small) so within-R2 can be computed
  # against the correct region-FE-only null after the loop, without
  # re-running the point-in-polygon join.
  attr(m, "n_regions") <- data.table::uniqueN(d_fe$grid_id)
  attr(m, "wr2_data") <- d_fe[, .(grid_id, ln_ntl)]
  m
}
...
hr_small <- data.table::fread(here::here("01_datasets/processed/hr_excluded_small_countries.csv"))
...
cat("\n=== Export Col(4)-(7) own-estimate rows (feeds table4_assemble_hrsample.R) ===\n")
# HR's exact-sample models (pop<500k + above-65N excluded) are what the
# paper's own Table IV replication reports; models_full/models_hr126 are
# robustness variants discussed in prose, not part of the assembled table.
# Within R2, HR/Stata xtreg-fe convention: 1 - deviance(m) / deviance(a
# null model with only the grid-cell fixed effect). Same convention as
# Table II and Table IV Col(1)-(3) -- NOT fixest::r2(m, "wr2"), which nets
# out both fixed effects and is mechanically near zero here.
wr2_grid <- function(m) {
  dd <- attr(m, "wr2_data")
  null_dev <- stats::deviance(fixest::feols(ln_ntl ~ 1 | grid_id, data = dd))
  1 - stats::deviance(m) / null_dev
}
own_rows_grid <- data.table::rbindlist(lapply(names(models_hrexact), function(res) {
  m <- models_hrexact[[res]]
  cft <- summary(m)$coeftable
  pv <- cft[1, 4]
  sig <- ifelse(pv < 0.01, "***", ifelse(pv < 0.05, "**", ifelse(pv < 0.10, "*", "")))
  data.table::data.table(
    col = which(c("50", "100", "200", "400") == res) + 3,
    b = cft[1, 1], se = cft[1, 2], sig = sig,
    n_regions = attr(m, "n_regions"),
    n_obs = nobs(m),
    within_r2 = wr2_grid(m),
    source = "own"
  )
}))
data.table::setorder(own_rows_grid, col)
data.table::fwrite(own_rows_grid, file.path(out_dir, "table4_col4_7_own_hrsample.csv"))
cat(sprintf("Saved: %s\n", file.path(out_dir, "table4_col4_7_own_hrsample.csv")))
