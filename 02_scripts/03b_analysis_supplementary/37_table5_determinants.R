...
#
# Description:   Full replication of HR 2014 Table V (Determinants of
#                Regional Favoritism, p. 1020-1022) on the authors' exact
#                126-country, 1992-2009 sample. Spec: ln_ntl ~
#                l(is_birthregion) * X | gid_2 + gid_0^year, for X in
#                {Polity, Schooling, NationalGDP, Language, FamilyTies}
#                one at a time (Columns 1-5), plus a Column (6) with all
#                five together. HR's notation lags the time-varying
#                covariates (Polity/Schooling/NationalGDP) one period,
#                same as Leader itself; Language and FamilyTies are
#                time-invariant, so carry no lag.
#
#                Data source deviations from HR 2014 (documented, same
#                pattern as Table II Column 8's G-Econ substitution):
#                NationalGDP uses a more recent Penn World Table vintage
#                than HR's own Heston, Summers & Aten (2012) PWT 7.1;
#                FamilyTies is this project's own reconstruction from WVS
#                microdata (waves 1-4, 1981-2004 only), since neither of
#                Alesina & Giuliano's own papers publishes a raw country
#                lookup table.
#
...
# Outputs:       01_datasets/processed/ntl/table5_determinants_models.rds
# ==============================================================================
...
cat("\n=== Table V regressions ===\n")
# HR's notation is Leader_ict-1 x Polity_ct-1 / Schooling_ct-1 /
# NationalGDP_ct-1: the time-varying covariates are ALSO lagged one
# period, not just Leader. Language_c and FamilyTies_c carry no time
# subscript (time-invariant), so are correctly left unlagged.
...
m5 <- fixest::feols(ln_ntl ~ fixest::l(is_birthregion) * family_ties | gid_2 + gid_0^year,
                     data = d_fe, vcov = ~spell_cluster)
# Column (6): all five determinants together (HR p. 1022, "In column
# (6), we look at all five potential determinants together").
...
cat("\n=== Comparison with HR 2014 Table V (p. 1021) ===\n")
