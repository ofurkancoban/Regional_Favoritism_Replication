# R/17_table4_col3_final.R
# Final, exact replication of HR 2014 Table IV Col(3): SN1 regions, NTL
...
#
# Unlike the earlier approximation in R/14_table4_col23.R (unweighted mean
# of ADM2 zonal means, used only because a real hole-punched extraction
# wasn't available yet), this version uses a genuine GEE zonal-mean
# extraction over hole-punched ADM1 geometry (birth ADM2 sub-polygons
# geometrically removed via st_difference, full precision, no simplification
# -- see R/15_build_holepunched_adm1.R and R/16_gee_adm1_holepunched.R).
#
# Only 581 of 3,610 ADM1 regions actually contain an ever-birth ADM2 and
# needed hole-punching; all others are identical to the Col(2) full-area
# value and are taken directly from data/processed/ntl/adm1_stable_global_panel.csv.
...
adm1_full <- data.table::fread("data/processed/ntl/adm1_stable_global_panel.csv")
adm1_full <- adm1_full[year %between% c(1992L, 2013L)]
data.table::setnames(adm1_full, c("GID_1", "iso3"), c("gid_1", "gid_0"))
adm1_full <- unique(adm1_full[, .(gid_1, gid_0, year, dmsp_ntl_full = dmsp_stable_lights)], by = c("gid_1", "year"))

cat("\n=== Load hole-punched extraction (real Col(3) values for the 581 affected ADM1s) ===\n")
holed <- data.table::fread("data/processed/ntl/adm1_holepunched_stable_global_panel.csv")
